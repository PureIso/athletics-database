...
namespace AthleticsDatabase
{
    ...
    public class SqLiteDatabase
    {
        ...
        public SqLiteDatabase(string databaseName)
        {
            try
            {
                _connection = "Data Source=" + Application.StartupPath + Path.DirectorySeparatorChar + databaseName + ".db;Version=3; password =x0x0";
                _databaseConnection = new SQLiteConnection(_connection);
        ...
        public bool ValidateUser(string userName, string userPassword)
        {
            ...
            {
                //Experimental
                using (var databaseCommand = new SQLiteCommand(sqlQuery, _databaseConnection))
                                      {
                                          var databaseDataReader = databaseCommand.ExecuteReader();

                                                  //Check if we have a valid name
                                                  if (databaseDataReader != null && databaseDataReader.HasRows != true)
                                                  return false;

                                                  sqlQuery = ("SELECT * FROM admins WHERE adminName = '" + userName +
                                                  "' AND adminPassword = '" + userPassword + "'");
                                                  if (ConnectionState.Open.ToString() != "Open") return false;
                                                  using (var databaseCommand2 = new SQLiteCommand(sqlQuery,
                                          _databaseConnection))
                                                  {
                                                      databaseDataReader = databaseCommand2.ExecuteReader();

                                                      //Check if we have a valid name and password - if the database has row then its valid
                                                      return databaseDataReader == null ||
                                                             databaseDataReader.HasRows;
                                                  }
                                      }
            }
            catch (Exception e)
        ...
        public IEnumerable<string> ListAllEvents(Enumerations.CompetitionType competitionType)
        {
            try
            {
                ...
                var databaseCommand = new SQLiteCommand(sqlQuery, _databaseConnection);
                var databaseDataReader = databaseCommand.ExecuteReader();
        ...
        public IEnumerable<string[]> SearchAthletes(string name, string surname, Enumerations.Gender gender)
        {
            try
            {
                ...
                var databaseCommand = new SQLiteCommand(sqlString, _databaseConnection);
                var databaseDataReader = databaseCommand.ExecuteReader();
        ...
        public int GetDatabase()
        {
            ...
            try
            {
                const string sqlQuery = "SELECT version FROM databaseVersion";
        ...
        public void UpdateDatabase(int version)
        {
            ...
            try
            {
                var sqlQuery = "Update databaseVersion SET version = " + version +"";
        ...
        private int GetNextCompetitionId()
        {
            try
            {
                ...
                var databaseCommand = new SQLiteCommand(sqlQuery, _databaseConnection);
                var value = databaseCommand.ExecuteScalar().ToString();
        ...
        public void AddCompetition(string competitionName, string competitionCountry, string region, string venue, string date, Enumerations.CompetitionType competitionType)
        {
            ...
            try
            {
                ...
                var sqlQuery = "SELECT * FROM competitions "
                    ...
                    + "AND competitionSeason = " + date.Substring(6, 4) + " AND competitionDate = '"+date+"' ";
                ...
                databaseCommand = new SQLiteCommand(sqlQuery, _databaseConnection);
                databaseCommand.ExecuteNonQuery();
            }
            catch (Exception e)
            {
        ...
        public void AddResults(string selectedEvent, string performance, int athleteId, int competitionId, Enumerations.Gender gender, Enumerations.CompetitionType competitionType, int season)
        {
            try
            {
                var eventId = GetEventId(selectedEvent, competitionType);
                ...
                if (gender == Enumerations.Gender.Male)
                {
                    if (competitionType == Enumerations.CompetitionType.Indoor)
                    {
                        sqlQuery = "INSERT INTO resultsmaleindoors(resultsMaleIndoorID, performance, competitionID, athleteMaleID, eventID) "
                                   + "Values (" + nextId + ", '" + performance + "'," + competitionId + "," + athleteId + ", " + eventId + ") ";
                    }
                    else
                    {
                        sqlQuery = "INSERT INTO resultsmaleoutdoors(resultsMaleOutdoorID, performance, competitionID, athleteMaleID, eventID) "
                                   + "Values (" + nextId + ", '" + performance + "'," + competitionId + "," + athleteId + ", " + eventId + ") ";
                    }
                }
                else
                {
                    if (competitionType == Enumerations.CompetitionType.Indoor)
                    {
                        sqlQuery = "INSERT INTO resultsfemaleindoors(resultsFemaleIndoorID, performance, competitionID, athleteFemaleID, eventID) "
                                   + "Values (" + nextId + ", '" + performance + "'," + competitionId + "," + athleteId + ", " + eventId + ") ";
                    }
                    else
                    {
                        sqlQuery = "INSERT INTO resultsfemaleoutdoors(resultsFemaleOutdoorID, performance, competitionID, athleteFemaleID, eventID) "
                                   + "Values (" + nextId + ", '" + performance + "'," + competitionId + "," + athleteId + ", " + eventId + ") ";
                    }
                }
                                ...
                                if (String.CompareOrdinal(performance, databaseTime) > 0)
                                {
                                    if (eventId == 9 || eventId == 10 || eventId == 11 || eventId == 12 || eventId == 13 || eventId == 14)
                                    {
                                        sqlQuery = "UPDATE athletesmaleseasonsbestindoors "
                                            + "SET bestID = " + currentDatabaseId + ", performance = '" + performance + "', eventID = " + eventId + ", athleteMaleID = " + athleteId + ", competitionID = " + competitionId + " , season = " + season + " "
                            ...
                            if (String.CompareOrdinal(performance, databaseTime) > 0)
                            {
                                if (eventId == 9 || eventId == 10 || eventId == 11 || eventId == 12 || eventId == 13 || eventId == 14)
                                {
                                    sqlQuery = "UPDATE athletesfemaleseasonsbestindoors "
                                               + "SET bestID = " + currentDatabaseId + ", performance = '" + performance + "', eventID = " + eventId + ", athleteFemaleID = " + athleteId + ", competitionID = " + competitionId + " , season = " + season + " "
                                               + "WHERE bestID = " + currentDatabaseId + "";
                                    databaseCommand = new SQLiteCommand(sqlQuery, _databaseConnection);
                                    databaseCommand.ExecuteNonQuery();
                                    return;
                ...
                if (gender == Enumerations.Gender.Male)
                {
                    ...
                    {
                        sqlQuery =
                            "INSERT INTO athletesmaleseasonsbestindoors(bestID, performance, eventID, athleteMaleID, competitionID, season) ";
                        sqlQuery += "Values (" + nextId + ",'" + performance + "'," + eventId +
                                    "," + athleteId + "," + competitionId + ",  " + season + ")";
                    }
                    else
                    {
                        sqlQuery =
                            "INSERT INTO athletesmaleseasonsbestoutdoors(bestID, performance, eventID, athleteMaleID, competitionID, season) ";
                        sqlQuery += "Values (" + nextId + ",'" + performance + "'," + eventId +
                                    "," + athleteId + "," + competitionId + ",  " + season + ")";
                    }
                }
                else
                {
                    ...
                    {
                        sqlQuery =
                            "INSERT INTO athletesfemaleseasonsbestindoors(bestID, performance, eventID, athleteFemaleID, competitionID, season) ";
                        sqlQuery += "Values (" + nextId + ",'" + performance + "'," + eventId +
                                    "," + athleteId + "," + competitionId + ",  " + season + ")";
                    }
                    else
                    {
                        sqlQuery =
                            "INSERT INTO athletesfemaleseasonsbestoutdoors(bestID, performance, eventID, athleteFemaleID, competitionID, season) ";
                        sqlQuery += "Values (" + nextId + ",'" + performance + "'," + eventId +
                                    "," + athleteId + "," + competitionId + ",  " + season + ")";
                    }
                }
        ...
        public void Close()
        {
            if (_databaseConnection.State != ConnectionState.Open) return;
            _databaseConnection.Close();
        }
        /// <summary>Open Connection</summary>
        public void Open()
        {
            if (_databaseConnection.State != ConnectionState.Closed) return;
            _databaseConnection.Open();
        }
        #endregion
    }
